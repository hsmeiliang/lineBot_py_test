    ...
    def askDietRecord(event):
        ...
        if(meal == 4):
            while(meal != 0):
                ...
                'queryTime': datetime.now() - dt.timedelta(hours = query_time)
                }
                response = requests.post(config.PHP_SERVER+'mhealth/food/queryFoodRecord.php', data = parm)
                resultList = json.loads(response.text)
                message.append(FlexSendMessage(alt_text=day_text[day] + query_meal[meal] + '紀錄', contents=BubbleMsg.todayDiet(query_meal[meal], resultList, day)))
            ...
            response = requests.post(config.PHP_SERVER+'mhealth/food/queryFoodRecord.php', data = parm)
            resultList = json.loads(response.text)
            message = FlexSendMessage(alt_text=day_text[day] + query_meal[meal] + '紀錄', contents=BubbleMsg.todayDiet(query_meal[meal], resultList, day))
